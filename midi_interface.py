...
class HXStompMidi:
    ...
    Optionally opens a MIDI input port alongside the output to listen for
    program changes and snapshot changes sent by the hardware, enabling
    two-way sync (call ``start_listening()`` after connecting).
    """
    ...
    def __init__(self, port_name: str = None, channel: int = 0):
        ...
        self._device_callback: Callable[[str, dict], None] | None = None
    ...
    @staticmethod
    def find_hx_input_port() -> str | None:
        """Auto-detect the HX Stomp MIDI input port by name substrings."""
        keywords = ("hx stomp", "hx-stomp", "line 6", "line6", "helix")
    ...
    # ------------------------------------------------------------------
    # Two-way device sync (MIDI input listener)
    # ------------------------------------------------------------------
    ...
    def start_listening(
        ...
        """Open a MIDI input port and start a background listener thread.

        The *callback* is called (from the listener thread) with two arguments:

        * ``event_type`` — one of ``"preset_change"``, ``"snapshot_change"``,
          or ``"disconnected"``.
        * ``data`` — a dict whose keys depend on *event_type*:

          * ``preset_change``: ``{"preset": int, "bank_msb": int, "bank_lsb": int}``
          * ``snapshot_change``: ``{"snapshot": int}``
          * ``disconnected``: ``{}``

        The listener automatically retries after a disconnect / port-closed
        error (2-second back-off), so USB unplugging and replugging is handled
        without user interaction.

        Parameters
        ----------
        callback :
            Callable invoked on each device event.  Must be thread-safe
            (e.g. dispatch to the UI thread via ``widget.after(0, …)``).
        input_port_name :
            Substring of the MIDI input port name to use.  When *None*,
            :meth:`find_hx_input_port` is tried first, then the connected
            output port name is used as a fallback.
        """
        self.stop_listening()
        self._device_callback = callback

        if input_port_name is None:
            input_port_name = self.find_hx_input_port() or self._port_name
    ...
    def stop_listening(self) -> None:
        """Stop the background MIDI input listener, if running."""
    ...
    def _listen_loop(self, port_name: str) -> None:
        """Background thread: read MIDI input and fire the device callback."""
        bank_msb = 0
        bank_lsb = 0

        while self._device_callback is not None:
            in_port = None
            try:
                available = mido.get_input_names()
                candidates = [p for p in available if port_name.lower() in p.lower()]
                if not candidates:
                    time.sleep(2.0)
                    continue

                in_port = mido.open_input(candidates[0])
                self._in_port = in_port
                print(f"[HXStomp] Listening on input: {candidates[0]}")
                ...
                for msg in in_port:
                    ...
                    if msg.type == "control_change":
                        if msg.control == CC_BANK_MSB:
                            ...
                            bank_lsb = msg.value
                        elif msg.control == CC_SNAPSHOT and msg.value < 8:
                            cb = self._device_callback
                            if cb:
                                cb("snapshot_change", {"snapshot": msg.value})
